...
* **콜아웃(Callout) 지원**:
    * Obsidian의 Callout 문법(`> [!Type]`)을 네이버 블로그 호환 테이블 디자인으로 자동 변환
    * Blue, Cyan, Green 등 3가지 색상 테마 및 아이콘 자동 매핑
    * 제목 내 마크다운 문법(`**Bold**`, `` `Code` ``) 완벽 지원

## 🆕 v1.2 업데이트 (2026-01-23)

* **선택 영역 복사 (Selection Copy)**:
    * 문서 전체가 아닌 드래그한 **텍스트 일부만** 변환하여 복사하는 기능 추가
* **콜아웃(Callout) 기능 강화**:
    * **웹앱(Web App)**에서도 Obsidian과 동일한 콜아웃 변환 로직 적용
